...
Anonymous research code and frozen-membership metadata for the paper
*Sustainable Responsible-AI Evaluation: When Compute Savings Change Benchmark
Conclusions*.

Author names, affiliations, and identifying links are withheld for anonymous
review.

Responsible-AI (RAI) evaluation has a computational footprint, yet efficiency is
...
Instead, `data/manifests/` contains metadata-only frozen evaluation manifests.
They identify the exact source records used in this paper without carrying any
benchmark payload. Obtain the original datasets from their official sources
under their original terms, then use the manifests and `REPRODUCIBILITY.md` to
reconstruct and verify the same evaluation membership.
...
2. Prepare the frozen records using the metadata-only manifests in
   `data/manifests/`; do not substitute or augment records.
...
See `LICENSE.md` and the upstream dataset projects for applicable terms.
